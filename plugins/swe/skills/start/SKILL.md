...
- **Regole di pre-selezione DETERMINISTICHE** (stesso stato ⇒ stesso default tra istanze diverse):
  ...
  - **Numero sessione**: **NON si calcola qui.** Si usa esclusivamente il designatore restituito dal gate (§4, `session-gate.mjs --mode=check`). Se il gate non ha emesso PASS non esiste un numero e la sessione non si apre: nessuna deduzione dal `SESSION_LOG`, nessun conteggio, nessun fallback. La regola "ultima entry + 1" e' ABROGATA (§4, misurata in S204).
