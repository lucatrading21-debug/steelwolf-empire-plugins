...
- **Per progetto**: hub/predator → `hub/steelwolf-empire-hub/SESSION_BRIEFINGS`; ta-* → il loro repo (path-set §0-ter). **Bot-Alliance** e ogni progetto `swe_writes:true` (ta-*, journal) → RICEVONO il modello come hub/predator: blocco ```swe-model``` completo nel loro `S<n+1>_OPEN.md` (BA-Alliance in `hub/steelwolf-empire-hub/SESSION_BRIEFINGS/BA-S<n+1>_OPEN.md`; ta-* nel loro repo, path-set §0-ter). SENZA modello -> card a mano NON deterministica (bug S52 BA): quindi il modello e' OBBLIGATORIO per tutti gli swe_writes:true. Solo i domini ESTERNI (`swe_writes:false`, repo:null: nexus/workdash) NON ricevono il modello, e comunque swe non li apre affatto.
